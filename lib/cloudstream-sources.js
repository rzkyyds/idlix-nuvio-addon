...
const CATALOG_VERIFY_LIMIT = parseInt(process.env.CLOUDSTREAM_CATALOG_VERIFY_LIMIT || "8", 10);
...
const SOURCES = [
  ...
  },
  {
    ...
    // Filmkita is allowed in catalogs, but cards/streams are byte-checked before display.
    catalogVerifyStreams: true,
    verifyStreams: true,
    disabledCatalogs: ["series"],
    types: ["movie", "series"],
...
async function catalogFromSource(source, bucket = "movie", page = 1) {
  ...
  const metas = $(source.itemSelector).toArray()
    .map((el) => {
  ...
  if (!source.catalogVerifyStreams) return metas;

  const verified = [];
  for (const meta of metas.slice(0, Math.max(1, CATALOG_VERIFY_LIMIT))) {
    const streams = await getStreams(meta.id);
    if (streams.some((stream) => stream.url && !stream.externalUrl)) verified.push(meta);
  }
  return verified;
}
...
async function isPlayableStreamUrl(url) {
  if (!url) return false;
  try {
    const { status, data, headers } = await http.get(url, {
    ...
    const head = Buffer.from(data).slice(0, 32);
    if (/mpegurl|m3u8/i.test(contentType) || head.toString("utf8").startsWith("#EXTM3U")) return true;
    if (/video\//i.test(contentType) || head.includes(Buffer.from("ftyp"))) return true;
    return false;
  } catch (err) {
    console.error(`[cloudstream] stream verify failed ${url}: ${err.message}`);
    return false;
  }
}

async function verifyPlayableStreams(streams) {
  const verified = [];
  for (const stream of streams) {
    if (!stream?.url || stream.externalUrl) continue;
    if (await isPlayableStreamUrl(stream.url)) verified.push(stream);
  }
  return verified;
}
...
async function getStreams(id) {
  ...
  if (directStreams.length) {
    const cleaned = filterStreams(directStreams).slice(0, 30);
    return source.verifyStreams ? await verifyPlayableStreams(cleaned) : cleaned;
  }
