...
async function catalogFromSource(source, bucket = "movie", page = 1) {
  ...
  return $(source.itemSelector).toArray()
    .map((el) => toMetaPreview(source, $, el, requestBase, bucket === "series" || bucket === "ott" ? "series" : "movie"))
    .filter(Boolean);
}
...
async function getCatalog(bucket, type, page = 1) {
  const enabled = SOURCES.filter((source) => source.types.includes(type) || bucket === "ott");
...
async function search(query, type = "movie") {
  ...
  const enabled = SOURCES.filter((source) => source.types.includes(type));
...
async function getStreams(id) {
  ...
  const streams = raw.filter((url) => {
    ...
    return {
      name: `${source.name}\n${host}`,
      title: `${source.name} • Server ${idx + 1}\n${host}`,
      ...(isDirect ? { url } : { externalUrl: url }),
      behaviorHints: {
        notWebReady: !isDirect,
        bingeGroup: `cloudstream-${source.id}-${host}`,
      },
    };
  });
...
function getSourceSummary() {
  return SOURCES.map(({ id, name, repo, defaultBaseUrl, websiteJsonKey, types, home }) => ({
    id,
    name,
    repo,
