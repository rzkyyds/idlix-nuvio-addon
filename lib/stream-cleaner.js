...
function isBlockedStream(stream) {
  ...
  return isNsfw(stream) || isTooLarge(stream) || BLOCKED_STREAM_TERMS.some((term) => text.includes(term));
}
...
function scoreStream(stream) {
  ...
  if (/\bfslv?2?\b|cdn\.fsl|fsl-buckets|fukggl|gigabytes/i.test(text)) score += 180;
