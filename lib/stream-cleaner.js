...
function isArchiveInsteadOfVideo(stream) {
  const url = String(stream?.url || stream?.externalUrl || "");
...
function isBlockedStream(stream) {
  ...
  return isNsfw(stream) || isTooLarge(stream) || hasKnownUnauthorizedHost(stream) || isArchiveInsteadOfVideo(stream) || BLOCKED_STREAM_TERMS.some((term) => {
...
function scoreStream(stream) {
  ...
  // FenixFlix is Brazilian: "Dublado" means Portuguese dub, while "Legendado"
  // normally keeps the original audio with Portuguese subtitles. Prefer original
  // audio when both variants share a resolution; keep dubbed as coverage fallback.
  if (stream?.behaviorHints?.upstreamKey === "fenixflix") {
    if (/\blegendad[oa]s?\b/i.test(text)) score += 260;
    if (/\bdublad[oa]s?\b/i.test(text)) score -= 260;
  }
...
function cleanStreamList(streams, { onePerResolution = true } = {}) {
  ...
  const hasFenixOriginalAudio = streams.some((stream) => (
    stream?.behaviorHints?.upstreamKey === "fenixflix"
    && /\blegendad[oa]s?\b/i.test(textOf(stream))
  ));
  ...
  for (const raw of streams) {
    ...
    if (
      hasFenixOriginalAudio
      && raw?.behaviorHints?.upstreamKey === "fenixflix"
      && /\bdublad[oa]s?\b/i.test(textOf(raw))
    ) continue;
