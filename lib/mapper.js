...
function findBestMatch(results, targetTitle, targetYear, targetType) {
  ...
  const scored = results.map((item) => {
    ...
    // Coerce years to strings — Cinemeta returns string years, IDLIX returns numbers
    if (String(iYear) === String(targetYear)) score += 20;
    // Prefer items that actually have a year over null/undefined
    if (iYear != null && targetYear != null) score += 5;
  ...
  // When scores tie (e.g. both 115), prefer the one with a year set
  if (scored.length >= 2 && scored[0].score === scored[1].score) {
    const aYear = scored[0].item.year;
    const bYear = scored[1].item.year;
    if (aYear == null && bYear != null) {
      // Swap: prefer item with year
      const tmp = scored[0];
      scored[0] = scored[1];
      scored[1] = tmp;
    }
...
function toStreams(videoSources, { slug, bingeGroup, proxyBase } = {}) {
  ...
  return sources.map((source) => {
    ...
    const stream = {
      name, title,
      url: finalUrl,
      // Send to external player (VLC, MX Player) — CDN anti-hotlinking
      // blocks headless TLS; real video players have browser-grade fingerprint.
      behaviorHints: { bingeGroup: group, notWebReady: true },
    };
