...
function looksLikeSpecificWrongTitle(stream, expectedTitle) {
  ...
  // These upstreams resolve by IMDb ID but label rows with localized Portuguese titles.
  // Do not throw them away just because Cinemeta's English title differs.
  if (["fenixflix", "froststream"].includes(stream?.behaviorHints?.upstreamKey)) return false;
