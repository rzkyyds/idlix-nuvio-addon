...
function movieboxReferer(subjectId, detailPath = "") {
  return `${WEB_API}/spa/videoPlayPage/movies/${detailPath}?id=${subjectId}&type=/movie/detail&lang=en`;
}
...
async function getMeta(id) {
  ...
  const meta = {
    ...
    website: movieboxReferer(parsed.subjectId, subject.detailPath || ""),
  };
  if (type === "series") {
...
async function getStreams(id) {
  ...
  const streams = await cachedJson(cacheKey, 120, async () => {
    const referer = movieboxReferer(parsed.subjectId, subject.detailPath || "");
  ...
  const direct = streams
    ...
    .map((stream, index) => ({
      ...
      behaviorHints: {
        ...
        proxyHeaders: {
          request: {
            Referer: movieboxReferer(parsed.subjectId, subject.detailPath || ""),
            "User-Agent": USER_AGENT,
          },
        },
      },
